...
const createTaskService = async (workspaceId, projectId, userId, body) => {
    ...
    if (!project || project.workspace.toString() !== workspaceId.toString()) {
        throw new apiErrors_1.NotFoundException("Project not found or does not belong to this workspace");
    }
    if (assignedTo) {
...
const updateTaskService = async (workspaceId, projectId, taskId, body) => {
    ...
    if (!task || task.project.toString() !== projectId.toString()) {
        throw new apiErrors_1.NotFoundException("Task not found or does not belong to this project");
    }
    const updatedTask = await task_model_1.default.findByIdAndUpdate(taskId, {
        ...body,
    }, { new: true });
    if (!updatedTask) {
        throw new apiErrors_1.BadRequestException("Failed to update task");
    }
    return { updatedTask };
};
...
const getTaskByIdService = async (workspaceId, projectId, taskId) => {
    ...
    if (!task) {
        throw new apiErrors_1.NotFoundException("Task not found.");
    }
    return task;
};
...
const deleteTaskService = async (workspaceId, taskId) => {
    ...
    if (!task) {
        throw new apiErrors_1.NotFoundException("Task not found or does not belong to the specified workspace");
    }
    return;
};
