...
const getMemberRoleInWorkspace = async (userId, workspaceId) => {
    ...
    if (!workspace) {
        throw new apiErrors_1.NotFoundException("Workspace not found");
    }
    ...
    if (!member) {
        throw new apiErrors_1.UnauthorizedException("You are not a member of this workspace", error_code_enum_1.ErrorCodeEnum.ACCESS_UNAUTHORIZED);
    }
...
const joinWorkspaceByInviteService = async (userId, inviteCode) => {
    ...
    if (!workspace) {
        throw new apiErrors_1.NotFoundException("Invalid invite code or workspace not found");
    }
    ...
    if (existingMember) {
        throw new apiErrors_1.BadRequestException("You are already a member of this workspace");
    }
    const role = await roles_permission_model_1.default.findOne({ name: role_enum_1.Roles.MEMBER });
    if (!role) {
        throw new apiErrors_1.NotFoundException("Role not found");
    }
