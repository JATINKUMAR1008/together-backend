...
const account_providers_enum_1 = require("../enums/account.providers.enum");
const loginOrCreateAccountService = async (data) => {
    ...
    try {
        ...
        if (!user) {
            ...
            if (!ownerRole) {
                throw new apiErrors_1.NotFoundException("Owner role not found");
            }
...
const registerUserService = async (body) => {
    ...
    try {
        ...
        if (existingUser) {
            throw new apiErrors_1.BadRequestException("Email already exists");
        }
        ...
        const account = new account_model_1.default({
            userId: user._id,
            provider: account_providers_enum_1.ProviderEnum.EMAIL,
            providerId: email,
        });
        await account.save({ session });
        // 3. Create a new workspace for the new user
...
const verifyUserService = async ({ email, password, provider = account_providers_enum_1.ProviderEnum.EMAIL, }) => {
    const account = await account_model_1.default.findOne({ provider, providerId: email });
    if (!account) {
        throw new apiErrors_1.NotFoundException("Invalid email or password");
    }
    const user = await user_model_1.default.findById(account.userId);
    if (!user) {
        throw new apiErrors_1.NotFoundException("User not found for the given account");
    }
    const isMatch = await user.comparePassword(password);
    if (!isMatch) {
        throw new apiErrors_1.UnauthorizedException("Invalid email or password");
    }
